...
仮定として、256bit のハッシュ値はすべての値が同じ確率で出現する（一様分布）ものとします。
...
誰でも１つ、決まっている誕生日というもの、うるう年を無視すれば
365 通り存在することになります。これが仮に均等な確率で決まっているとすると、
ある日付の誕生日である確率は 1/365 しかないので、
...
人為的に TXID を衝突させることができてしまうという問題が存在します。
...
さて、BIP-30 の導入によって、人為的にコインベーストランザクションの
TXID を衝突されることはできなくなりました。同時にバースデイパラドックスでの
...
新規に参入したノードが祖先ブロックから順に同期をする場合など、
以前とは比較にならないほど遅くなってしまったのです。

そこでその問題を解決しようと提案されたのが BIP-34 です。

「エクストラ Nonce 領域は、0x03 で開始して、続けて
