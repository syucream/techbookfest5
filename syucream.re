...
ここではまず、マイクロサービスの世界における課題を解決するソフトウェアである Envoy と、それを利用した Istio について触れます。
...
ただしスレッドローカルストレージとは言ったものの、 pthread の pthread_get_specific() などの API を使っているわけではなく、共有のデータ構造にスレッド毎にスロットを割り当てて、各スレッドが自身のスロットのみを利用するようにしています。
...
分散システムにおいてしばしば障害点を切り離して全体の動作をなるべき維持するサーキットブレイカーを導入することがあると思われます。
