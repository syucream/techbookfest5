...
//image[syucream_monolith_overview][モノリシックアーキテクチャのイメージ][scale=0.7]
...
またモノリシックであるがゆえ、エラーハンドリング漏れや遅延の発生がアプリケーション全体に影響しかねません。
...
筆者としてはマイクロサービスアーキテクチャが馴染まないケースが多々存在すると考えています。
これに従うにしても組織としてある程度覚悟をし、リスクを承知の上で取り掛かる必要があるでしょう。
...
しかし幸いなことに、それらの問題を解決するソフトウェアも世に多く公開されています。
...
例えばこれが 1 つのモノリシックなシステムであれば開発時にデバッグをしたければ従来のデバッガを使ったり、静的解析してコールグラフを出したりスタックトレースを出力してみたりすることができます。
